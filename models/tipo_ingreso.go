...
// AddTipoIngreso insert a new TipoIngreso into database and returns
...
// GetTipoIngresoById retrieves TipoIngreso by Id. Returns error if
// Id doesn't exist
...
// GetAllTipoIngreso retrieves all TipoIngreso matches certain condition. Returns empty list if
// no records exist
func GetAllTipoIngreso(query map[string]string, fields []string, sortby []string, order []string,
	offset int64, limit int64) (ml []interface{}, err error) {
	o := orm.NewOrm()
	qs := o.QueryTable(new(TipoIngreso))
	// query k=v
	for k, v := range query {
...
// UpdateTipoIngreso updates TipoIngreso by Id and returns error if
// the record to be updated doesn't exist
func UpdateTipoIngresoById(m *TipoIngreso) (err error) {
	o := orm.NewOrm()
	v := TipoIngreso{Id: m.Id}
...
// DeleteTipoIngreso deletes TipoIngreso by Id and returns error if
...
func DeleteTipoIngreso(id int) (err error) {
	...
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Delete(&TipoIngreso{Id: id}); err == nil {
